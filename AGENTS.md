...
└── website/
    ...
    └── src/
        ...
        ├── assets/
        │   ├── player/            # head/body/left-arm/right-arm .png — the Mii figure, cut in Figma (see below)
...
- **No active section (`/`):** a full-viewport `Court` (that's the entire landing — no name/typewriter/ticker block) — an SVG tennis court centred in a widened landscape viewBox. Its four navigation targets (positions in `SECTIONS`/`BOXES` in `src/data/sections.js`) are **targets, not click-to-navigate buttons**: the two **far service boxes** (across the net) are Experience/Projects, and the two **circular archery targets beyond the far baseline** (off the right side of the court) are About/Contact. `Court` renders its zone `<button>`s `disabled` on the hub, since navigation happens by aiming and launching the ball, not by clicking. The server's run-up space to the left of the court hosts the `OutCall` banner.
...
## Player assets — why they're PNG, not SVG

Figma exported the four player parts as `.svg`, but each one **embedded the entire 286×513 source PNG** (identical base64 in all four files) and used a mask to reveal one limb. That shipped the same photo four times — ~142 KB per part, ~568 KB total, for four small crops.

They are now flat transparent PNGs, cropped to each part: **~95 KB total, an 83% cut.** They're raster either way — the SVG wrapper bought nothing.

If you re-cut the character in Figma and get masked SVGs again, rasterize before committing. Render each SVG at its natural `viewBox` size on a transparent background:

```bash
"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" \
  --headless=new --disable-gpu --default-background-color=00000000 \
  --window-size=<svg-width>,<svg-height> --screenshot=part.png "file://$PWD/part.html"
```

where `part.html` is a bare page containing just `<img src="part.svg">` sized to the same dimensions. The natural sizes are already several times the on-screen size (a 46-unit-wide head renders at ~57 CSS px), so 1× is plenty — don't render at 2×. The original masked SVGs remain in git history if you ever need them back.
